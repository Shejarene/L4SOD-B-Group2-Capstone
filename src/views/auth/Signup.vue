...
const verifyInvite = async () => {
  try {
    const { data } = await api.get(`/auth/invites/verify/${route.params.token}`)
    if (data.success) {
      invite.value = data.data
      if (data.data.email) form.email = data.data.email
    } else {
      error.value = data.message || 'Invalid or expired invite link'
    }
  } catch (err) {
    error.value = err.response?.data?.message || 'Invalid or expired invite link'
...
const handleSignup = async () => {
  ...
  try {
    await api.post(`/auth/invites/accept/${route.params.token}`, form)
    toast.add({ severity: 'success', summary: 'Account created! You can now sign in.', life: 5000 })
    router.push('/login')
  } catch (err) {
    toast.add({ severity: 'error', summary: 'Error', detail: err.response?.data?.message || 'Signup failed', life: 3000 })
